...
describe('jsonl', () => {
  ...
  it('parses well-formed lines with line numbers', () => {
    ...
    expect(parsed).toEqual([
      { kind: 'value', line: 1, value: { n: 1 } },
      { kind: 'value', line: 2, value: { n: 2 } },
    ])
  })
  ...
  it('skips blank lines but preserves numbering', () => {
    ...
    expect(parsed).toEqual([
      { kind: 'value', line: 1, value: { a: 1 } },
      { kind: 'value', line: 4, value: { b: 2 } },
    ])
  })
  ...
  it('reports corrupt lines without aborting the rest', () => {
    ...
    expect(parsed[0]).toEqual({ kind: 'value', line: 1, value: { good: 1 } })
    ...
    expect(parsed[2]).toEqual({ kind: 'value', line: 3, value: { good: 2 } })
  })
