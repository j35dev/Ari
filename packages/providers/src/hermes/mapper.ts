 ...
 * throws. The native schema (lines typed system|assistant|user|result;
 * assistant content blocks text/thinking/tool_use; result carrying usage +
 * total_cost_usd) is synthesized after the Claude Code stream-json shape —
 * the hermes CLI was unavailable locally, so __fixtures__ are hand-written
 * against this documented schema and must be re-recorded via
 * scripts/record-fixture.ts once the CLI ships.
 */
...
export function mapHermesLine(line: string): AgentEvent[] {
...
export function mapHermesStream(lines: Iterable<string>): AgentEvent[] {
  return Array.from(lines).flatMap((line) => mapHermesLine(line))
}
