...
import { mapHermesLine, mapHermesStream } from './mapper'
...
describe('hermes mapper', () => {
  it('maps a successful session end-to-end', () => {
    ...
    if (usage?.type === 'usage') {
      ...
      expect(usage.costUsd).toBeCloseTo(0.0031)
    }
  })

  it('maps the recorded model_not_found error fixture to an error event + done', () => {
    const events = mapHermesStream(fixture('error-session.jsonl'))
  ...
  it('never throws on malformed lines — surfaces an error event instead', () => {
    const events = mapHermesLine('{not json')
  ...
  it('maps tool errors with isError=true', () => {
    ...
    const events = mapHermesLine(line)
