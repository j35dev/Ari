...
- [x] M24.7 Terminal docks to the right rail instead of taking the page (user request,
      2026-09-03 — reverses M24.1): clicking Terminal (or Ctrl+`) opens the machine's own
      shell beside the transcript, T3 Code style, spawned in the active project's folder.
      Tabs, not splits: `terminal-dock.ts` is a module-level store so closing the rail parks
      the shells instead of killing them (only a tab's × kills its pty) and a reopened rail
      re-adopts them, with background tabs mounted-but-`invisible` so output keeps flowing
      and xterm's fit addon still measures them. The whole rail is now drag-resizable
      (`use-pane-width.ts` generalises the sidebar hook; the trailing edge inverts the drag,
