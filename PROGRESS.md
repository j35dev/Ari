...
- [x] M26.2 Wallpaper visibility presets: Subtle / Balanced / Vivid segmented control persisted as `appearance.wallpaperLook` (superseded by M26.5 — the presets and the setting are gone; one uniform glass look remains).
- [x] M26.3 Vivid readability fix from live testing: the per-column reading frost is replaced by the main pane itself carrying the sidebar's glass recipe (superseded by M26.5's single plate).
- [x] M26.4 Vivid polish + trim: pane material mirrored the sidebar's glass-on/off state, sidebar/main hairline border added, Subtle retired (superseded by M26.5).
- [x] M26.5 One uniform wallpaper look, no visibility setting: the whole content area is a single `.ari-glass-pane` plate carrying the sidebar's exact recipe (`--ari-glass-scrim` + blur(28px)), applied to the shell, settings, and gallery roots — so settings looks like the workspace and no pane is darker or blurrier than its neighbor. Nested `.ari-glass`/`.bg-bg` fills go transparent inside the plate, which removes both the stacked tint and the `backdrop-filter` fringe that showed as a slit in the sidebar/main gutter; structure now comes from hairline borders. `appearance.wallpaperLook` removed from contracts (zod strips the stale key) and from the provider/cache; `wallpaper-css.test.ts` guards the CSS structure.
