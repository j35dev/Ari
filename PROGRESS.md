...
- [x] M33.6 The key typed into the settings form is actually sent. Fetching models and testing a connection both passed `apiKey: null`, so a new endpoint — whose key exists only in the form, nothing saved yet — got a 401 from any keyed provider on the exact flow the feature exists for. Keyless and local servers were unaffected, which is why tests missed it. Both calls now forward a non-empty key field and pass the endpoint id while editing so the engine can fall back to the stored key; discovery takes an explicit `persist` flag instead of inferring intent from a missing id, so the form can supply its id for key lookup without writing an unsubmitted endpoint to the store. `endpoints.test` now probes the same listing path discovery uses, so a green test means a fetchable model list.
- [x] M33.7 Read-only tool calls run concurrently. The loop awaited each call in turn, so the fan-out that opens every exploration (four reads and a grep) paid five sequential round-trips. Tools declare themselves side-effect free via `readOnly` — set on the built-in read/grep/glob/ls — and those run as one batch, while mutating and external tools stay strictly ordered because their sequence is the model's intent and two writes to one file must not race. Results are yielded in call order either way, so the transcript is identical. A guarded name is never fanned out even if a tool claims `readOnly`, so the flag cannot be used to skip the approval gate.
- [x] M33.8 Context is summarized, not just dropped. Trimming deleted old turns and left `[earlier tool results trimmed]`, so a long session lost what it had decided and why. Past 75% of the budget the older span now goes through the session's own model into a fixed structure (goal, constraints, progress, decisions, next steps, critical context) while the newest 35% of the budget stays verbatim. Compaction runs between rounds, never mid-round where a tool call may lack its results, and the cut lands on a user message so the kept span always starts a turn. The summary replaces the span in the stored transcript too, so the next turn starts from it. Failure falls back to trimming — a degraded context beats a failed turn. Known gap: summarization usage is not folded into turn totals, so cost under-reports slightly when a session compacts.
