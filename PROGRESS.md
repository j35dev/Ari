...
- [x] M26.3 Vivid readability fix from live testing: the per-column reading frost is replaced by the main pane itself carrying the sidebar's glass recipe (`--ari-glass-scrim` + blur(28px)), so chat/usage/changes text over a crisp wallpaper reads exactly like the sidebar does.
- [x] M26.4 Vivid polish + trim: the main pane mirrors the sidebar's *actual* material (glass recipe with `data-ari-glass='on'`, opaque `--ari-surface-0` with it off) instead of always blurring, closing the "vivid pane is blurrier than the sidebar" gap; the main pane gains a `border-l` hairline so the sidebar/main resize gutter no longer shows a see-through slit; the Subtle look is retired (two-look control) with a settings preprocess mapping stale 'subtle' values to 'balanced'.
