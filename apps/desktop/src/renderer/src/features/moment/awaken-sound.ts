 ...
 * Every failure path is swallowed: a launch must never break over audio.
 */
...
export function playAwakenSound(): () => void {
  ...
  try {
    ...
    noiseGain.gain.linearRampToValueAtTime(0.045, now + 0.16)
    noiseGain.gain.exponentialRampToValueAtTime(0.0001, now + 0.4)
    noise.connect(filter).connect(noiseGain).connect(ctx.destination)
    noise.start(now)

    tone(ctx, now + 0.55, 0.65, 261.63, 0.028) // C
    tone(ctx, now + 0.67, 0.72, 329.63, 0.03) // E
    tone(ctx, now + 0.8, 0.92, 392.0, 0.026) // G
    ...
    shimmerGain.gain.linearRampToValueAtTime(0.022, now + 1.38)
