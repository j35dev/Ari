...
export const themePersistence: ThemePersistence = {
  async load() {
    ...
    const { themeId, mode, wallpaper } = settings.appearance
    return { themeId, mode, wallpaper }
  },
  async save({ themeId, mode, wallpaper }) {
    await rpc.invoke('settings.update', { appearance: { themeId, mode, wallpaper } })
