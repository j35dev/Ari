...
export const themePersistence: ThemePersistence = {
  async load() {
    ...
    const { themeId, mode, glass, wallpaper, wallpaperLook } = settings.appearance
    return { themeId, mode, glass, wallpaper, wallpaperLook }
  },
  async save({ themeId, mode, glass, wallpaper, wallpaperLook }) {
    await rpc.invoke('settings.update', {
      appearance: { themeId, mode, glass, wallpaper, wallpaperLook },
    })
